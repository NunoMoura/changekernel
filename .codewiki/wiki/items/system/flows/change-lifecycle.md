...
Review compares the exact integrated Change tip with the Committed Change and current canonical Project head. A passed current Gate allows Project Server to create the Completion Commit under expected-head CAS. Failure returns work to the responsible Work Unit, Planning, or a Superseding Change; stop preserves state. Delivery follows as a separately authorized protected effect and does not redefine local completion.
