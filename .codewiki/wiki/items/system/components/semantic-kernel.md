...
`src/kernel/data-contracts/` owns bounded data representation and validation shared by Kernel domains. `canonical-json.ts` defines canonical values and deterministic JSON encoding; `validation.ts` validates fields, structures, and exact protocol identities; `outcome.ts` provides typed success/failure values. These mechanisms neither decide lifecycle policy nor own canonical Git state. Canonical encoding remains a precise data-format term, not the name of this broader responsibility.
