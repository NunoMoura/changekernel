...
Alignment is the condition where relevant accepted desired state and realized project state are connected, bound to an active Change, or explicitly unknown. Knowledge State is authoritative for desired meaning; source, tests, exact project snapshots, and delivery bindings establish realized artifacts and bytes. Alignment compares those categories and never treats either one as proof of the other.

Decision may commit desired checkpoint `K1` while the current realization remains `R0`. Alignment must report that exact pending, partial, blocked, stale, or unknown gap until Review proves a compatible `R1`; it never delays semantic acceptance merely to make both heads appear synchronized. Realization and delivery status are derived projection facts, not edits to accepted Knowledge. The target Semantic Kernel extends the same lineage from exact Raw Data Revisions through Wiki facets to Project Artifacts, Evidence, and Delivery without making the graph canonical authority.
