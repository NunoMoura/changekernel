...
DSH is an exact pinned upstream Plugin ecosystem composed behind CodeWiki contracts. The qualified managed-Run baseline is the atomic 26-package `0.1.1-rc.2` family with Cordis `4.0.1` and Cordis Plugin Loader `1.0.2`; package-lock v3 binds artifact integrity, and the reviewed source relationship remains explicitly unattested. Release-managed DSH profile bundles select separate trusted-host, Run Process, and future client contributions. The Runtime Bridge maps admitted Run Requests to DSH Agent, AgentLoop, Agent Session, and DSH Plugin composition inside isolated Run Processes; the private broker may separately compose provider, authorization, and credential Plugins outside those sandboxes. DSH owns no Project Server, accepted Knowledge, Client authority, Check Result, Gate, transition, or protected effect.
