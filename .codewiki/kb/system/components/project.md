...
Migration follows roadmap dependencies rather than combining unrelated persisted cuts. SK2 freezes repository/object format/canonical ref and performs one stopped, backup-tagged expected-old-ref Git commit from `kb/**` to `wiki/items/**`, rejects dual roots, and removes Domain plus custom state/projection/checkpoint authority. SK2 Receipts bind active-Change plans/ref names; Git binds OIDs. One release-managed compatibility component may retain legacy Pack, Check, source-provider, or project behavior; active Build binds only generic implementation/capability digests, and legacy Domain identity is execution Evidence. SK4 migrates stage-first Packs, removes Pack Skills, moves compatibility to fixed rules, Checks, config, or admitted external-mechanics Plugins, deletes residual Domain behavior, and activates Plugin Protocol/SDK. SK5 activates Raw Data custody and `source.observe`; ongoing connection state remains external, while admitted Revisions may stale support and open intake but never update Wiki automatically. Each cut has its own backup, collision check, expected Build, recovery transaction, Receipt, and rollback point; no cut dual-reads or dual-writes replaced authority. Migration is stopped-only. The first target-only accepted operation is a later accepted Change/work/protected-effect Git commit or Receipt that source protocols cannot represent; migration and maintenance audit Receipts do not count. Old-Build restore is allowed only before that boundary. Afterward recovery must retain canonical first-parent history, terminal proposal ancestry, and fixed-path Trace blobs or repair forward.
