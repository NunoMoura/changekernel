...
Project Server freezes Gate Evaluation Package Protocol `2.0.0` at Candidate checkpoint. It binds the exact subject, complete Check Pack, WorkState, Knowledge, Alignment, repository, Evidence, Result, configuration and route heads, all declared selections and execution identities, and complete Check coverage. Decision also binds its revision, Effects, base and projected Knowledge, compiler, plan, semantic view, and active-Change coverage. Other stages bind accepted Knowledge targets, requirements, repository tree/base, and realization lineage. Producer snapshots never become implicit Check input. Construction rejects unresolved handles, missing or unknown Checks, incomplete or stale selections, stage mismatch, and identity drift.
