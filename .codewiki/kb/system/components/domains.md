...
Identity binding: `domainCompilerIdentity(admission)` derives the exact Knowledge compiler identity from an admitted plugin, so checkpoints compiled by a plugin are transitively bound to its admission digest; the built-in plugin's derived identity equals the historical kernel default, proving existing checkpoints are already bound without byte drift. `assertCheckpointBoundToDomain` rejects checkpoints compiled by any other or drifted compiler identity. Project configuration selects one exact plugin through `wiki_config.domain.pluginId`; `null` means the release default and an unadmitted ID fails closed at resolution.
