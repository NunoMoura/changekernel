...
Project Server runs the Decision Gate over the exact Decision Candidate and a frozen Gate Evaluation Package. The editable default Decision Check Pack combines deterministic accepted Effect and invariant indexing with an `active_change_compatibility` Model Check. Complete coverage accounts for every accepted nonterminal Change; only shared Effect targets, shared invariants, explicit relationships, and unknown semantics expand full revisions into model context. The package binds those expansions, index-only proofs, explicit relationships, accepted Work Graph projection, and `acceptedActiveChanges` digest. The invariant is no unresolved semantic contradiction, not no overlap. Dependencies and technical ordering remain Planning concerns; resource contention remains a scheduler concern.
