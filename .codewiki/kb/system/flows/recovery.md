...
Backend Fault Recovery `1.0.0` closes crash, cancellation, timeout, quota, credential rotation, stale Session, Broker Host loss, filesystem corruption, process orphan, and partial effect. Runtime recovers only complete evidence, proves cancellation closure, and terminates descendant containment. Broker Host owns bounded transport retry and credential replacement. Project Server requests user authority, rolls over stale Sessions, quarantines/restores bytes, and reconciles idempotent effects. No fault path changes meaning, fabricates authority, silently selects another route, or repeats a protected effect without exact evidence.
