...
The exact SK0 baseline `e3e06723242b4115c28a9a716fd817ae23f439f7` and SK1 contract laboratory `5735315aa6d220115f033f863a4fc5c63ed2694b` passed external governance. Dogfood-readiness successor `32752d604e26ed7f759e6fa0bbf529097c537360` passed exact-subject qualification under activated controller `ed85062f181652e5083210ac666bdb5fb74e46a8` and was explicitly activated for private local external installation. SK2 candidate `81e542516ea7ebf51c37d1dd0beeae293f2c09a5` passed exact-subject qualification under controller `32752d604e26ed7f759e6fa0bbf529097c537360` and was explicitly activated, but migration of this repository failed closed before ref activation because its oversized canonical Lexicon was compiled as an ordinary Item instead of the promised omitted/folded Lexicon input. The target root remains absent. The correction must compile legacy Lexicons separately, use canonical JSON identity for migration plans and Receipts whose serialized Item files legitimately exceed one semantic scalar's 16 KiB bound, and disable rename pairing while parsing migration endpoint status. Candidate `4a6b8578719315d81c5c6ca6898ef63e469f473b` passed internal gates but exact-source external activation exposed the rename-pair parser defect, so it is rejected. A corrected candidate must pass fresh external exact-subject qualification including this repository's full canonical KB activation, then receive new explicit activation before migration resumes. `SEMANTIC_KERNEL_SK0_CONTRACTS.md` records the contract ledger, transformation inventory, protocol versions, migration and rollback edges, release gates, scenario fixtures, and retained evidence.
...
- [x] Stage and verify the backup ref, Domain-free target config, target Item/Trace objects, Receipt, migration commit, target Backend Build binding, and active proposal commits without advancing canonical or managed refs; remove the backup ref if final closure validation fails.
- [x] Atomically activate canonical and managed refs, materialize only migration paths, replay restart closure and private activation Receipt, activate the Domain-free target Build/private generation, and retain bounded recovery/rollback.
