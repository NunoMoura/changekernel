...
function isPortableScope(value: string): boolean {
	return value.length > 0 && value.length <= 512 && value.normalize("NFC") === value && !value.startsWith("/") && !value.includes("\\") && !value.includes("\0") && value.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== "..");
}
