...
import {canonicalJson, decodeCanonicalValue, isCanonicalObject, type CanonicalValue} from "../../kernel/data-contracts/canonical-json.ts";
import {failure, success, type Outcome} from "../../kernel/data-contracts/outcome.ts";
import {createChange, createProfileChange, type Change, type ChangeRelation, type ChangeTarget} from "../../kernel/changes/contracts.ts";
import {
	CHANGE_EVENT_KINDS,
	createChangeEvent,
	createProfileChangeEvent,
	type ChangeEvent,
...
import {
	...
	createProfileChangeTraceHeader,
	type ChangeTrace,
} from "../../kernel/changes/trace.ts";
import {decodeProjectSnapshot, validateChangeCommitSnapshot, validateCompletionCommitSnapshot, type ProjectSnapshot, type TreeChange} from "../../kernel/changes/snapshot.ts";
...
export async function executeLifecycleCommand(
	...
	if (environment.configuration.wikiProfile === WIKI_PROFILE_ID && operation !== "changes.propose-profile") {
		return failure(productError("unavailable", "This operation is unavailable while the Project uses its explicit Wiki profile.", "Use profile-native Change proposal in this lifecycle slice.", false));
	}
	if (operation === "changes.propose-profile" && environment.configuration.wikiProfile !== WIKI_PROFILE_ID) {
		return failure(productError("unavailable", "Profile-native Change proposal is unavailable for this Project.", "Select an explicitly profile-enabled Project.", false));
	}
	try {
		switch (operation) {
			case "changes.propose": return proposeChanges(environment, actor, input as ProposeChangesInput);
			case "changes.propose-profile": return proposeProfileChange(environment, actor, input as ProposeProfileChangeInput);
...
async function proposeProfileChange(
	...
	if (environment.configuration.wikiProfile !== WIKI_PROFILE_ID || input.profile !== WIKI_PROFILE_ID || environment.configuration.retiredWikiItemIds.length !== 0) {
		return failure(productError("unavailable", "Profile-native Change proposal is unavailable for this Project.", "Select an explicitly profile-enabled Project.", false));
	}
	...
	const before = await loadProfiledWikiSource(environment.store, environment.configuration, {kind: "commit", commit: input.expectedProjectHead}, WIKI_PROFILE_ID, profileSourceLimits(environment.configuration));
	if (!before.ok) return failure(invalidRequest(`Exact before-source admission failed (${before.error.operation}: ${before.error.code}).`));
	const after = await loadProfiledWikiSource(environment.store, environment.configuration, {kind: "commit", commit: input.afterCommit}, WIKI_PROFILE_ID, profileSourceLimits(environment.configuration));
	...
	const committed = await appendTraceCommit(environment, {
		...
		message: `codewiki: propose profile Change ${input.changeId}\n`,
	});
	...
	], actor.authorizationId, "codewiki: propose Change with unchanged Project head");
...
function validateStageState(
	...
	if (stage === "review") return change.review === null ? failure(conflict("Review Checks require current canonical reconciliation.")) : success(true);
