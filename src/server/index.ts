...
import {decodeProductReadInput, type ChangesReadInput, type ProductReadInput, type ProductReadOperation, type ProjectSourceSelector} from "../api/contracts/read.ts";
import {createProductTransportResponse, decodeProductTransportRequest, productError, type ProductError, type ProductTransportRequest, type ProductTransportResponse} from "../api/transport/envelope.ts";
...
import {MAX_TRACE_BYTES, PROFILE_CHANGE_TRACE_PROTOCOL} from "../kernel/changes/trace.ts";
import {resolveKernelWikiContract, WIKI_PROFILE_LIMITS} from "../kernel/wiki/profile.ts";
...
import {PROJECT_ACCESS_POLICY_PROTOCOL, profileScopeGuard, type AuthorizedProjectActor, type ProjectAccessPolicy} from "./authorization/policy.ts";
import {discoverProject, readProjectCapabilities} from "./queries/project.ts";
import {executeLifecycleCommand} from "./commands/lifecycle.ts";
import {readProfileChange} from "./queries/profile-change.ts";
import {resolveProjectSource, type ProjectReadConfiguration, type ProjectReadLimits, type ProjectSourceIssue} from "./queries/source.ts";
...
export interface ProjectServerFoundation {
	...
	readonly capabilities: Readonly<{projectStore: "available"; agentRuntime: "available"; preview: "available" | "unavailable"}>;
}
...
export interface ProjectServerProject {
	...
	/** Defaults to this Kernel release. Omission never selects a different document contract. */
...
const DEFAULT_LIMITS: ProjectReadLimits = Object.freeze({
	maximumWikiItems: WIKI_PROFILE_LIMITS.files,
	maximumWikiFileBytes: WIKI_PROFILE_LIMITS.fileBytes,
	maximumWikiTotalBytes: WIKI_PROFILE_LIMITS.contextBytes,
...
export function bindProjectServerFoundation(
	...
	if (typeof ports !== "object" || ports === null || !hasOnlyKeys(ports, ["projectStore", "agentRuntime", "preview"]) ||
...
export function createProjectServer(
	...
	if (typeof input !== "object" || input === null ||
		!hasOnlyKeys(input, ["ports", "accessPolicy", "project", "limits", "maximumReplayEntries", "clock"])) {
	...
	const server = Object.freeze({
		...
		async handle(raw: unknown): Promise<unknown> {
			...
			try {
				...
				const availability = operationAvailability(request.value.operation, request.value.input);
				if (availability !== null) return responseFor(request.value, failure(availability));
				const scope = request.value.operation.startsWith("changes.") ? profileScopeGuard(actor.value, request.value.operation, request.value.input) : null;
				if (scope !== null) return responseFor(request.value, failure(scope));
				...
				if (commandOperations.has(request.value.operation as ProductCommandOperation)) {
					...
					const outcome = await executeLifecycleCommand({
						...
						committer: Object.freeze({name: "ChangeKernel Project Server", email: "changekernel@localhost"}),
					...
					response = responseFor(request.value, outcome, prepared.value.binding);
				}
...
function operationAvailability(operation: string, input: CanonicalValue): ProductError | null {
	if (operation === "project.discover" || operation === "project.capabilities" || operation === "changes.propose-profile") return null;
	if (operation === "changes.read" && isCanonicalObject(input) && input.view === "get") return null;
	return productError("unavailable", "This operation is not implemented by the current Kernel.", "Use Project discovery, capabilities, managed-document proposal or Changes get.", false);
}
...
async function executeRead(
	...
	if (request.operation === "project.discover") return success(discoverProject(configuration));
	if (request.operation === "project.capabilities") return success(readProjectCapabilities(actor));
	if (request.operation === "changes.read" && (input as ChangesReadInput).view === "get") {
		return readProfileChange(store, configuration, actor, input as Extract<ChangesReadInput, {view: "get"}>);
	}
	return failure(productError("unavailable", "This read is not implemented by the current Kernel.", "Choose an available read.", false));
}
...
async function prepareRead(
	...
	const base = Object.freeze({
		...
		derivation: Object.freeze({kernelBuildDigest: configuration.kernelBuildDigest, kernelVersion: configuration.kernelVersion}),
		interpretation: Object.freeze({changeTrace: Object.freeze({...PROFILE_CHANGE_TRACE_PROTOCOL})}),
		bounds: canonicalMetadata(configuration.limits),
	});
	if (request.operation === "project.discover" || request.operation === "project.capabilities") {
		return success(Object.freeze({input, binding: Object.freeze({...base, source: null, freshness: "configuration"})}));
	}
	...
	return success(Object.freeze({
		input: Object.freeze({...sourceInput, source: Object.freeze({kind: "commit" as const, commit: snapshot.value.commit})}) as ProductReadInput,
		binding: Object.freeze({...base, source: canonicalMetadata(snapshot.value), freshness: "exact"}),
	}));
}
...
function projectConfiguration(
	...
	const required = ["projectName", "repositoryId", "objectFormat", "canonicalRef", "kernelBuildDigest"];
	const allowed = [...required, "kernelVersion"];
	if (typeof project !== "object" || project === null || ![Object.prototype, null].includes(Object.getPrototypeOf(project)) ||
		!required.every(key => Object.hasOwn(project, key)) ||
		!Reflect.ownKeys(project).every(key => {
		...
		return failure(bindingFailure("invalid_configuration", "project", "Project configuration must contain only supported own-data fields; legacy selectors are not supported."));
	}
	if (typeof project.projectName !== "string" || project.projectName.length === 0 ||
		new TextEncoder().encode(project.projectName).byteLength > 256 || project.projectName.normalize("NFC") !== project.projectName ||
		/[\u0000-\u001f\u007f-\u009f\u202a-\u202e\u2066-\u2069]/u.test(project.projectName) || !isNamespacedIdentifier(project.repositoryId)) {
	...
	const contract = resolveKernelWikiContract(Object.hasOwn(project, "kernelVersion") ? project.kernelVersion : CHANGEKERNEL_VERSION);
	if (!contract.ok) return failure(bindingFailure("invalid_configuration", "kernelVersion", contract.error.message));
	...
	return success(Object.freeze({
		...
		kernelVersion: contract.value.kernelVersion,
		limits: limits.value,
	}));
}
...
function readLimits(input: Partial<ProjectReadLimits> | undefined): Outcome<ProjectReadLimits, ProjectServerBindingFailure> {
	...
	const bounds: readonly Readonly<{field: keyof ProjectReadLimits; minimum: number; maximum: number}>[] = [
		{field: "maximumWikiItems", minimum: 1, maximum: WIKI_PROFILE_LIMITS.files},
		{field: "maximumWikiFileBytes", minimum: 1, maximum: WIKI_PROFILE_LIMITS.fileBytes},
		{field: "maximumWikiTotalBytes", minimum: 1, maximum: WIKI_PROFILE_LIMITS.contextBytes},
