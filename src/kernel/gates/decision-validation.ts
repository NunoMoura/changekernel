...
import {CONTRACT, LIMITS, RESULT_FIELDS, admit, bounds, codec, compare, digest, digests, effectKinds, equal, fail, hash, inputSlots, list, record, resultFields, snapshot, subject, text} from "./evaluation-data.ts";
 ...
 * must supply current facts and retained domain results, verify source custody and
...
export function prepareDecisionValidation(input: unknown) {
	return decodeContract("Decision validation", input, value => {
		...
		const current = Object.freeze({snapshot: snapshot(requiredField(CONTRACT, c, "snapshot")), subject: subject(requiredField(CONTRACT, c, "subject")),
		...
		if (!equal(current.snapshot, current.subject.baseline)) fail("Decision baseline differs from the current governing snapshot.");
		...
		const domain = record(requiredField(CONTRACT, r, "domain"), ["selection", "results"]);
		const domainSelection = admit(prepareCheckSelection(domain.selection));
		const domainReduction = admit(reduceCheckResults(domain));
		for (const key of ["snapshot", "subject", "stage", "effects", "effectsComplete", "kernelBuildDigest"] as const) {
			if (!equal(current[key], domainSelection.current[key])) fail("Domain assessments differ from the exact current Decision subject or stage.");
		}
		const domainResults = Object.freeze([...list(domain, "results", value => admit(decodeCheckResult(value)))].sort((a, b) => compare(a.checkId, b.checkId)));
		const domainMaterial = Object.freeze({selectionDigest: domainSelection.digest, reduction: domainReduction,
			// Keep adopted questions, conditions, parameters, exact inputs and limits,
			// not only favorable result summaries. These are data, never instructions.
			selection: requiredField(CONTRACT, domain, "selection"), results: domainResults});
		...
		if (domainResults.some(result => result.evidenceDigests.some(digest => !evidenceDigests.includes(digest)))) missing.push("domain-evidence");
		if (!domainReduction.passed) missing.push("required-domain-assessment");
		...
		const inputs = Object.freeze(definitions.map(definition => {
			...
			const body = Object.freeze({protocol: DECISION_INPUT_PROTOCOL, owner: "backend", stage: "decision", kernelVersion: CHANGEKERNEL_VERSION,
				...
				configurationDigest: hash("changekernel.decision-validation-configuration@1.0.0", configuration), slots, domain: domainMaterial});
		...
		const identity = {protocol: "changekernel.decision-validation-selection@1.0.0", current, configuration, entries, domain: domainReduction};
