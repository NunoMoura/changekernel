...
This document records the architecture-complete SK0 freeze developed from clean baseline `62bb34d450b51bc0f9cfb26c37cd3af48e68a077`. The exact SK0 baseline at `e3e06723242b4115c28a9a716fd817ae23f439f7` was externally governed and qualified. Executable SK1 work then exposed one transitive self-reference in the migration Receipt's active-proposal binding: a Receipt embedded in the candidate commit cannot also hash proposal commit OIDs that depend on that candidate. This correction narrows only that impossible binding; it does not reopen the Git, Wiki Item, Change, authority, recovery, or one-writer decisions. `SEMANTIC_KERNEL_PLAN.md` remains the roadmap, `.codewiki/kb/**` remains the canonical product and system design, and Backend v1 source and tests remain executable truth until a qualified slice explicitly migrates them. This file is the cross-owner inventory and exact target-contract ledger; any conflict with a canonical KB owner blocks review and must be corrected rather than resolved by precedence at runtime.

Nothing in this document activates `.codewiki/wiki/**`, changes a persisted protocol, or permits dual authority. Every target version below is reserved for a future executable slice. Qualification evidence remains valid only for its exact committed subject; this corrected contract must enter the next externally governed candidate and does not inherit the prior commit's qualification automatically.
...
Protocol versions MUST remain persistence and compatibility identities, not semantic source-code identities. Ordinary source filenames and exported types, functions, variables, services, and test helpers MUST use stable semantic-owner names without `V<major>`, version numbers, or equivalent suffixes. Version-specific names are permitted only at an explicit compatibility dispatcher, migration adapter, persisted protocol constant, or fixture whose purpose is identifying that exact format. Thus the semantic API is `createChangeTraceOperation`, while `codewiki.change-trace@13.0.0` remains the persisted protocol value. Clean-break activation replaces the owner implementation rather than introducing a parallel version-branded API.
...
| Qualified KB migration | legacy Domain migration `1.0.0` | plan `codewiki.kb-to-wiki-migration@1.0.0`, proof `codewiki.kb-to-wiki-legacy-equivalence@1.0.0`, and Receipt `codewiki.kb-to-wiki-migration-receipt@1.0.0` producing one Git migration commit | SK2 |
...
| T02 | `.codewiki/kb/**` + `.codewiki/traces/**` → Wiki Items + `.codewiki/changes/**` | F1, F2; Q1, Q2, Q9 | Knowledge State/Projection/Checkpoint and Trace `12.0.0` → Wiki Item, native proposal/disposition commits, managed refs, and Trace `13.0.0` | One quiescent backup-tagged `codewiki.kb-to-wiki-migration@1.0.0` Git commit after exact stub hydration; preserve legacy record map; pre-target-acceptance ref restore, then target-compatible restore or forward repair only; reject dual roots | G0–G4, G8–G10 |
...
The Receipt binds source commit/state/projection/Trace, every identity/path/provenance/retirement map, emitted Item blob/tree OIDs, every converted fixed-path Trace blob, the migration Change trace's pre-operation blob, Kernel Build, migration intent, implementation, and one conversion plan plus expected managed-ref name for each active Change. An active-Change conversion plan binds the exact converted Trace prefix, preserved semantic inputs and unresolved requirements, proposed Wiki bytes plus explicit migrated-Item deletions, and frozen commit inputs. It specifies that the resulting candidate becomes both proposal parent and `expectedCanonical`, but it does not contain the candidate OID or any candidate-dependent proposal commit, Trace, or tree OID. `migration.applied` then binds Receipt digest as the final migration Trace operation; neither it nor the Receipt contains the candidate canonical commit OID.

After the candidate exists, Project Server materializes each active proposal from its bound conversion plan, validates its exact Trace-prefix append, Wiki bytes, parent, `expectedCanonical`, and frozen commit inputs, then atomically advances canonical ref and creates every active managed Change ref with expected-old-OID checks while stopped. The resulting Git refs, commits, and retained ancestry are the exact proposal-OID proof; the Receipt does not duplicate that native Git authority. A legacy-equivalence reader reconstructs every compiled current subject, independently mutable cell, relationship, accepted body, and retirement from target Items plus maps. Kernel Validation reparses the candidate and proposal trees, checks Git object types and Item/reference/retirement closure, and proves WorkState equivalence before the ref transaction.

Crash before ref transaction leaves unreachable staging objects and source ref authoritative. Crash after the transaction is complete is recoverable from canonical and managed Git refs by revalidating them against the Receipt's conversion plans. Before any later target-only accepted commit, stopped rollback may restore the backup ref and old Build while preserving migration and rollback evidence. Afterward, only target-compatible current-history restore or forward repair is valid. The KB-to-Wiki converter, legacy-equivalence reader, and source-Build restore path are bounded transition machinery: they never participate in normal target lifecycle routing and remain available only for the qualified upgrade and rollback window before explicit retirement.
...
SK0 did not self-approve. Its documentation candidate was committed as one bounded N+1 checkpoint whose governance packet bound baseline and candidate commits, complete changed-file and diff digests, the contract-ledger digest, retained-source hashes, ambiguity resolutions, test and diagnostics Receipts, and an explicit declaration of its scope. The released controller governed that exact subject externally. Any later contract correction, including this migration Receipt correction, follows the same exact-subject rule and does not borrow the prior commit's result.

A released CodeWiki controller N installed outside this checkout in a disposable project governs each exact candidate and packet under protected-head policy. N records Decision, required Checks, Gate, confirmation, and immutable outcome Evidence without loading N+1 as this repository's Pi extension or writing dogfood state here. The original passing external Receipt permitted SK1 to begin; this correction must enter the next bounded candidate and qualification. Rejection amends documentation in another bounded candidate; it does not silently waive a gate. The external project and credentials are cleaned after Evidence retention, while this repository retains only reviewed source and Git checkpoint evidence. Repository dogfood remains prohibited until an immutable stable release, disposable migration/recovery rehearsal, and an explicit governed activation checkpoint establish external controller N for N+1; mutable candidate code never governs itself.

## SK0 exit and continuing correction gate

Architecture work is ready for review, and later corrections remain admissible, when:
