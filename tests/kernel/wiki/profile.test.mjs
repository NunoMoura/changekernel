...
function coreFiles() {
	return WIKI_CORE_TYPES.map((name, index) => profileFile(`.codewiki/wiki/types/${name}.md`, "TypeDefinition", name, oid(index + 1)));
}

test("profile identity and limits are immutable domain data", () => {
	assert.equal(WIKI_PROFILE.identifier, WIKI_PROFILE_ID);
	assert.equal(WIKI_PROFILE.id, "codewiki.wiki-profile");
	assert.equal(WIKI_PROFILE.version, "1.0.0");
	assert.equal(Object.isFrozen(WIKI_PROFILE), true);
...
test("metadata preserves exact text, unknown inert data, and prototype-looking keys", () => {
	...
	const decoded = decodeWikiMetadata(".codewiki/wiki/research/observation.md", fields("Claim", decomposed, {
...
test("metadata rejects reserved unknown fields and malformed optional fields", () => {
	for (const extra of [
		...
		const result = decodeWikiMetadata(".codewiki/wiki/item.md", input);
...
test("metadata copying applies cumulative UTF-8, node, depth, and collection budgets", () => {
	const tooMuchText = decodeWikiMetadata(".codewiki/wiki/item.md", fields("Claim", "Title", {
	...
	const many = decodeWikiMetadata(".codewiki/wiki/item.md", fields("Claim", "Title", {unknown: tooManyEntries}));
	assert.equal(many.ok, false);
	assert.equal(many.error.code, "limit_exceeded");

	let nested = "value";
	for (let index = 0; index <= WIKI_PROFILE_LIMITS.depth; index += 1) nested = {nested};
	const tooDeep = decodeWikiMetadata(".codewiki/wiki/item.md", fields("Claim", "Title", {unknown: nested}));
...
test("Change references resolve lexically and retain original spelling", () => {
	...
	const result = resolveChangeReference(".codewiki/wiki/research/item.md", original);
	assert.equal(result.ok, true, result.ok ? "" : result.error.message);
	assert.equal(result.value.reference, original);
	assert.equal(result.value.path, ".codewiki/changes/cafe\u0301");
	...
	for (const reference of [
		...
		const invalid = resolveChangeReference(".codewiki/wiki/item.md", reference);
		assert.equal(invalid.ok, false, reference);
	}

	const equivalent = decodeWikiMetadata(".codewiki/wiki/research/item.md", fields("Claim", "Title", {
...
test("custom TypeDefinitions require one direct non-meta core base", () => {
	for (const base of ["Definition", "Entity", "Contract", "Procedure", "Claim"]) {
		const valid = decodeWikiMetadata(".codewiki/wiki/types/FieldObservation.md", fields("TypeDefinition", "FieldObservation", {
			"codewiki-base": base,
		}));
	...
	for (const [path, title, extra] of [
		[".codewiki/wiki/types/FieldObservation.md", "FieldObservation", {}],
		[".codewiki/wiki/types/FieldObservation.md", "FieldObservation", {"codewiki-base": "TypeDefinition"}],
		[".codewiki/wiki/types/FieldObservation.md", "FieldObservation", {"codewiki-base": ["Claim"]}],
		[".codewiki/wiki/types/FieldObservation.md", "fieldObservation", {"codewiki-base": "Claim"}],
		[".codewiki/wiki/other/FieldObservation.md", "FieldObservation", {"codewiki-base": "Claim"}],
	]) {
...
test("type context binds six core types and custom instances deterministically", () => {
	...
	const customDefinition = profileFile(".codewiki/wiki/types/FieldObservation.md", "TypeDefinition", "FieldObservation", oid(20), {
		"codewiki-base": "Claim",
	});
	const instance = profileFile(".codewiki/wiki/fieldwork/marsh.md", "FieldObservation", "Marsh survey", oid(21));
...
test("type context enforces file-count and context-byte limits", () => {
	...
	const large = Array.from({length: 9}, (_, index) => {
		const path = `.codewiki/wiki/items/large-${index}.md`;
...
test("type context rejects missing/duplicate/unknown declarations, aliases, collisions, and bad snapshots", () => {
	...
	const unknown = [...definitions, profileFile(".codewiki/wiki/items/item.md", "MissingType", "Item", oid(41))];
	...
	const aliasOnly = [...aliasedDefinitions, profileFile(".codewiki/wiki/items/item.md", "ClaimAlias", "Item", oid(42))];
	assert.equal(bindWikiTypes(oid(1), aliasOnly).ok, false);
	const collision = [...definitions, profileFile(".codewiki/wiki/items/item.md", "Claim", "Item", oid(43)), profileFile(".codewiki/wiki/items/item.md/nested.md", "Claim", "Nested", oid(44), {
		"codewiki-origin": ["../../../changes/establish-profile"],
		"codewiki-revision": "../../../changes/establish-profile",
	})];
...
test("metadata rejects accessors, sparse arrays, cycles and non-inert values without mutation", () => {
	...
	for (const input of [accessor, fields("Claim", "Title", {extra: cyclic}),
		...
		assert.equal(decodeWikiMetadata(".codewiki/wiki/item.md", input).ok, false);
	}
...
test("metadata budgets count UTF-8 keys, values and nodes at exact boundaries", () => {
	...
	const atLimit = decodeWikiMetadata(".codewiki/wiki/items/item.md", input);
	assert.equal(atLimit.ok, true, atLimit.ok ? "" : atLimit.error.message);
	input.extra += "x";
	assert.equal(decodeWikiMetadata(".codewiki/wiki/items/item.md", input).error.code, "limit_exceeded");
	const nodes = Array.from({length: 9}, () => Array(1000).fill(null));
	assert.equal(decodeWikiMetadata(".codewiki/wiki/item.md", fields("Claim", "Title", {nodes})).error.code, "limit_exceeded");
});
...
test("SHA-256 context binds all core categories and rejects indirect custom bases", () => {
	...
	const instances = WIKI_CORE_TYPES.filter((type) => type !== "TypeDefinition").map((type, index) =>
		profileFile(`.codewiki/wiki/${type}.md`, type, type, oid(50 + index, "sha256"), {
	...
	const bad = decodeWikiMetadata(".codewiki/wiki/types/Special.md", fields("TypeDefinition", "Special", {"codewiki-base": "FieldObservation"}));
...
test("legacy envelope data remains rejected by the profiled metadata contract", () => {
	const legacy = decodeWikiMetadata(".codewiki/wiki/item.md", {
