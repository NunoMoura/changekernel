...
export function fixture({algorithm = "sha1", definition: definitionPatch = {}, adoption: adoptionPatch = {}, input: inputPatch = {}, selected: selectedPatch = {}} = {}) {
	const baseline = {repositoryId: "cw:repository:test", commit: oid("1", algorithm), tree: oid("2", algorithm)};
	const candidate = {...baseline, commit: oid("3", algorithm), tree: oid("4", algorithm)};
	...
	const adoption = ok(createCheckAdoption({wiki: {snapshot: baseline, path: ".changekernel/wiki/items/checks.md", blob: oid("5", algorithm)},
		adoptedBy: {snapshot: baseline, path: ".changekernel/changes/CHG-adopt/change.json", blob: oid("6", algorithm)}, reason: "Adopt the intent condition for Wiki proposals.",
	...
	const subject = {baseline, candidate, subjectDigest: digest("f")};
	...
	return {definition, pack, adoption, input, selection: {current: {snapshot: baseline, adoptionDigest: adoption.digest, kernelBuildDigest, subject, stage: input.stage, effects: input.effects, effectsComplete: input.effectsComplete, permissionDigests: [digest("d")]},
