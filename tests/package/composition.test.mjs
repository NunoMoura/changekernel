...
test("package metadata has one narrow export and an exact Pi runtime closure", async () => {
	...
	for (const [path, entry] of Object.entries(lock.packages)) {
		assert.equal(path.includes("node_modules/@deepseek-ai/"), false, path);
		if (/node_modules\/@earendil-works\/(?:pi-ai|pi-agent-core|pi-coding-agent|pi-tui|pi-telemetry|chord)$/u.test(path)) assert.equal(entry.version, "0.85.1", path);
	}
	assert.equal(pkg.peerDependencies, undefined);
	assert.equal(pkg.pi, undefined);
	assert.equal(pkg.private, true);
	assert.deepEqual(Object.keys(pkg.devDependencies).sort(), ["@types/node", "koffi", "typescript", "zod"]);
});
