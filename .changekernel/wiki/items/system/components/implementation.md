...
Agents perform authorized tool use, artifact creation and Evidence collection. Backend-owned Implementation validation and adopted domain Checks evaluate immutable submitted inputs without changing artifacts, Wiki, policy or Project Server state. Required failures return actionable feedback; unavailable execution is not a fabricated semantic verdict. Use cheap feedback and work-in-progress checkpoints during work. No new Change is required for every command or repair within accepted scope. An implementation defect stays here; a conflicting decomposition returns to Planning; material intent revision returns to Decision. Do not change the acceptance target silently to match produced bytes.

## Implementation validation boundary

The backend validates authorized assignment, readiness of prerequisites, exact attempt and artifact identity, execution/effect custody, and the relationship of actual contributions to accepted scope and Work. It assesses whether deviations and discoveries are accounted for, commitments remain respected and the claimed partial outcome is supported rather than inferred from an exit code or produced file. Domain Checks assess engineering results, experimental conduct or other project-specific execution standards. Both successful worker claims and routine in-scope repairs remain subject to their appropriate stage conditions; requiring approval for every command is not a substitute.

A Planning pass concerns a proposed path, not the quality of resulting work. Implementation binds actual contributions and observations; Review later assesses the joined outcome. Tests and fresh observations are produced through separately authorized work, not hidden side effects of a Check.
