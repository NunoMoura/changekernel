...
A PR merge flag, passing host check or producer receipt does not prove semantic acceptance. Assess the actual combined Proposed Change, not an isolated old head; hooks and client-side checks alone do not prevent bypass. Never execute untrusted proposed code with privileged credentials through a hosting workflow. Preserve portable records independently of host UI state.
