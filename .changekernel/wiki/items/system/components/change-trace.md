...
The early-inquiry representation is the same Change, not a new category. Its body binds protocol, Change/repository identity, revision, Current Project state, document-format identity (`profile`), intent and source basis, nullable rationale/scope/classification/realization, acceptance/questions/assumptions, source records, existing-kind relationships, Wiki consequences and the semantic digest. Revision starts at 1 and increments exactly once under the same Change/repository/recorded-format identity. The `profile` field is an internal record identity; representation and interpretation changes are versioned with the Kernel, not an independent profile release. A revision records the full replacement body and nonblank reason without rewriting earlier sources or event prefixes.

`wikiConsequences` distinguishes `{kind: "unresolved"}`, `{kind: "none", reason}` and `{kind: "profile", reference}`. None is a proposed conclusion, not a passed check. A profile attachment uses the existing exact reference contract and nonempty endpoint set; targets are derived, not fabricated for early intent. Its complete reference to Current Project state equals the revision’s recorded Current Project state; responsible path, profile and build grounds must match. Changed Current Project state/build requires explicit removal or reattachment. An intent revision cannot inherit an earlier Gate verdict even if its proposed bytes remain unchanged.
...
The [Check execution log](agent-execution.md#check-execution-log-and-change-trace) retains execution reservations and outcomes separately. The Change trace references the exact Check results used by a stage assessment, its governing Check policy and the transition or disposition that followed; it does not duplicate execution bookkeeping or become another Check result store. Retain completed results before publishing their Change-trace references. Recovery must verify an unreferenced result and current authority rather than rerun its Check automatically. Referenced results and necessary provenance must remain available, including when Change history is transported; missing records are explicit verification gaps, not permission to recreate judgments. This is the desired publication contract, not a claim that current proposal-only traces already implement it.
