...
For evaluation, supply bounded projections from the frozen authorized input capture with exact Wiki, Evidence, artifact, Change and configuration identities, source roles, timestamps and omissions. Code can inspect its permitted source capture; models receive bounded data without tools. A read does not adopt knowledge, repair sources, execute work or grant authority. Changing relevant sources, policy or validity conditions invalidates dependent reuse.
