...
The smallest backend composition connects the isolated Linux Check runner to one literal-loopback OpenAI-compatible route. Capture configuration before asynchronous setup; bind the endpoint, provider/model, transport bounds, sampling settings, verified backend build/dependency identity and an opaque backend-owned credential version. Do not publish credentials or a public hash of them. The trusted host verifies those identities and changes the credential binding when credentials change. Construction makes no inference call and grants no authority: domain launches require exact Check adoption; backend launches require the exact release-owned definition and verified Kernel Build. Both require readiness, bounded execution configuration and current permission. A loopback address does not prove that the service processes data locally; service behavior and surrounding host containment require separate qualification. This composition does not supply producer containment or permission to recover unresolved work.
...
## Check execution log and Change trace

Keep the Check execution log separate from the Change trace. The Check execution log owns execution reservations, completed results and stopped or unresolved outcomes. The Change trace owns the attributable lifecycle record: which exact Check results informed a stage assessment, the governing Check policy and exact assessed versions, and the resulting transition or disposition. Derived Change state references those records. Do not add another Check history store or maintain independently mutable copies of the same authoritative result. Backend validation results share the execution machinery while retaining their distinct ownership.

This separation keeps execution bookkeeping out of Change-trace writes and lifecycle replay, and supports exact retained-result lookup without traversing individual Change histories. It also creates retention and reference-integrity obligations. These are architectural reasons, not measured performance claims. Reuse across selections or Changes is never automatic: all exact-input, stage, evaluator, permission and current-authority requirements still apply.

The required write order is: durably reserve Check execution; execute and durably retain its settled outcome; then, under separate lifecycle authority, record a reference to that exact result in the Change trace. A crash after result retention but before Change-trace publication leaves an unreferenced result, not permission to repeat inference. Recovery may verify and reference it only when current source, Check policy and transition requirements permit; retention alone neither accepts Evidence nor advances the Change. An unresolved execution reservation blocks automatic retry.

Referenced Check results and their necessary provenance must remain recoverable for Change-history reconstruction. Retention or future archival must not invalidate live references. Portable Change history must carry or provide authorized durable access to the exact referenced results and required grounds; machine-local paths alone are insufficient. Missing or inaccessible records block verification and remain explicit gaps, never a reason to regenerate historical judgments with a model. Export must respect access restrictions rather than copying private execution storage wholesale.

A trace-backed execution design could replace separate storage only if it supplied equivalent reservation, exclusive execution, durable completion, access and recovery guarantees. Merely appending results after execution is insufficient. Current source implements private execution retention, not this complete Change-trace publication, transport or archival contract; those integrations remain unfinished.
