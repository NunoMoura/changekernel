...
Backend-owned stage validation assesses transversal conditions; adopted [domain Checks](checks.md) assess field-specific claims and requirements against exact policy and supplied Evidence. Agents gather information, prepare proposals and repair failures; both evaluation paths are read-only and may combine code with bounded model inference. An observation is information obtained by inspection, measurement or execution, not an additional authoring object. It becomes Evidence through its attributable use for a particular claim. [Evidence retention](evidence.md#committed-sources-and-durable-retention) binds committed Project sources and authorized external custody without treating Git as a proof of truth.

Domain Checks and Packs are custom, including first-party offerings. Common lifecycle validation belongs to the [ChangeKernel release contract](changekernel.md#backend-validation-across-four-stages), not project adoption; the boundary is transversal versus domain-specific, not mechanical versus semantic. Wiki records which exact domain policies, Check definitions/Packs and execution settings the project adopts, with scope, reasons and exceptions. Packs distribute evaluation methods, not an automatically governing second policy store. Prefer adopted policy parameters supplied to a reusable evaluator; an inherently executable rule can be adopted by exact reference rather than duplicated in prose. Installation, axis labels, Skills and proposed configuration grant no authority. Changes to policy or evaluator selection follow the previously adopted transition rules, not the Proposed Change's own waiver.

Managed-document interpretation and transversal stage validation are explicit backend contracts governed by the same ChangeKernel version. Domain policy cannot waive them, and their presence does not establish arbitrary domain assurance. No validation may be reported where none occurred. A retained backend or domain result belongs to an exact evaluation, not permanently to an Item. Any document coverage indicator is derived from those results, names the evaluator, owner, stage, sources, scope and current reuse eligibility, and does not mark the whole document true or verified. No visible or hidden frontmatter flag is required for result reuse.
...
Wiki revisions and Kernel versions are distinct. Editing prose or a TypeDefinition changes the Wiki revision under the existing rules. Changing those rules requires the corresponding major, minor or patch Kernel bump. Many Wiki revisions therefore use the same Kernel version, while historical interpretation remains bound to its exact recorded build and exact retained type definitions.
...
An exact Item is bound by Project state reference, path and retained bytes/blob identity. Changes preserve exact prior/resulting subjects and historical Change versions; a path alone is not an immutable receipt. Missing, private or reused historical targets remain unresolved rather than being guessed from the latest same-named path. Missing earlier authorship permits honest adoption of an observed initial Project state now, not invented ancient origins.
...
An authorized initial Change proposes the six core definitions, including TypeDefinition, under the selected immutable Kernel release's document contract. The Kernel supplies the non-waivable bootstrap floor; proposed definitions cannot authorize themselves. A custom type and its first instances may be one assessed Proposed Change under that floor. Type evolution requires impact assessment of affected Items, including byte-unchanged Items whose interpretation or obligations would change.

Bind interpretation to the exact Kernel Build and exact retained type definitions recorded for the subject, rather than repeating a fifth universal field on every Item or managing an independent profile version. Historical reads never substitute mutable latest definitions. A standalone exported file without that context remains material with unresolved version or acceptance. Exact binding/receipt encoding is an integration contract, not authority supplied by the document itself.
...
Cold information can include previously accepted knowledge, rejected alternatives and other sources. Cold does not mean false, unprocessed or never accepted. [Evidence](evidence.md) defines how retrieved material supports particular claims. Retrieval does not re-adopt an old rule, and missing origins require honest initial-state provenance rather than invented historical Changes.
