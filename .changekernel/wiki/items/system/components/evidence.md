...
One bounded information path serves current Wiki, native Git history, Project sources and explicitly authorized external connectors. External connectors are optional; the core lifecycle and history work without them. Archive and Sources are not separate knowledge systems. Discovery does not execute source instructions, grant permissions, adopt policy or disclose private material.

Record source roles, versions, omissions, redaction and freshness. Exact source caches, Evidence validity and evaluation-result reuse have different conditions; old bytes can remain valid historical support without establishing current behavior. Indexes, embeddings and generated summaries are rebuildable views, not substitutes for retained sources. Recovery preserves the recorded interpretation and its limitations rather than rerunning inference. Reuse of either backend or domain evaluations binds the entire supplied input capture, authority origin, stage question and evaluator context, not only the favorable sources cited in feedback. Git-backed comparison applies equally to retained Wiki and other Project artifacts, while source roles, selection scope and membership, custody, permissions and freshness remain relevant even when bytes match. Reuse retains the Boolean, feedback and Evidence references together; it creates no permanent verified flag on a document and no new authoritative cache store. See [Checks](checks.md#efficient-evaluation-and-reuse), [Project](project.md) and [recovery](../flows/recovery.md).
