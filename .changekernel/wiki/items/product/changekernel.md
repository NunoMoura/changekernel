...
The simple model is: **Wiki holds what we agree; agents propose and produce changes; ChangeKernel validates the four stages; adopted domain Checks assess project-specific requirements; ChangeKernel controls acceptance and remembers why.** Raw data supplies information, artifacts embody work, and [Alignment](../system/components/alignment.md) derives what is wanted, observed and being done about the gap. An observation is one way of obtaining Evidence, not another product object.

The [ChangeKernel backend](../system/components/changekernel.md#backend-validation-across-four-stages) owns transversal validation common to every domain, with explicit contracts for Decision, Planning, Implementation and Review. Domain-specific methods and criteria belong to adopted [Checks](../system/components/checks.md) and Packs, including first-party offerings. Either owner may use computation or bounded semantic assessment; domain Packs cannot waive the backend's requirements. Domain Checks are read-only functions using one library; TypeScript/JavaScript comes first, Python later. Wiki defines adopted project meaning and policy. Shared execution mechanisms preserve the different authority origins and stage questions, and reports name both kinds of coverage without exceeding their Evidence. Host instructions and Skills guide work but cannot replace enforcement. Public marketplace distribution is a later option, not a minimum-release dependency.
