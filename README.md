...
`BACKEND_V1_PLAN.md` owns the active production sequence. CodeWiki has removed Switchyard and its backend selector and now pins one exact replay-qualified DSH Plugin baseline. Before Backend v1 release, it must extract the Software Development Domain Plugin; qualify declared provider, API-key, OAuth, credential-custody, and containment combinations; freeze authenticated frontend API and event contracts; prove install, upgrade, rollback, backup, restore, crash recovery, diagnostics, and audit; and complete external release-N-governs-N+1 dogfood without loading mutable source-controller code.
