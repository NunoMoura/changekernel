...
- Branch: `audit/clean-baseline`; this source slice follows pushed design checkpoint `5b2b92edc`. Pi **0.85.1** remains pinned. The [transversal/backend and domain/Check boundary](.changekernel/wiki/items/system/components/changekernel.md#backend-validation-across-four-loops) is unchanged.
...
- **606/606 repository tests passed**, including type checks and build; log `/tmp/changekernel-decision-validation-full.log`. **158/158 installed-candidate tests passed**, using isolated settings, dummy credentials and loopback fixtures. All **134** installed distribution files match tested source build bytes; eight Pi dependency instances remain **0.85.1**. Candidate, lock and receipt: `/tmp/changekernel-decision-validation-proof-RwzKa0/`.
