...
Next: find a supported provider transport control, obtain an upstream change, or separately qualify containment that meets the [provider boundary](.changekernel/wiki/items/system/components/provider-boundary.md#effects-and-host-trust). Do not monkey-patch global network functions, vendor internals or write another client. Only then pin the chosen production dependency and wire an installer with exact settings, bounded usage and observable cancellation. Keep tests local and simulated, without live/paid inference. A small first-party Pack through the same interface and explicit adoption follows supported wiring.
